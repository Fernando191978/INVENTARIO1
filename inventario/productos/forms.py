...
class ProductoForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            'sku': forms.TextInput(attrs={'placeholder': 'Dejar vacío para generar automáticamente',
                'class': 'form-control'}),
        }
        labels = {
    ...
    def clean_stock(self):
        ...
        if stock and stock < 0:
            raise ValidationError("No puede haber valor negativo en stock.")
        return stock

    def clean_stock_minimo(self):
        stock_minimo = self.cleaned_data.get('stock_minimo')
        if stock_minimo and stock_minimo < 0:
            raise ValidationError("No puede haber valor negativo en stock mínimo.")
        return stock_minimo
